...
print("#######################")
plt.legend()
plt.title("Elo distribution after 400 games played in random pairs")
############################################################################################################
players = copy.deepcopy(all_players)
for _ in range(100):
    # reset elo
...
for player, info in players.items():
    ...
    print(player, np.mean(info["elos_per_competition"]))
plt.legend()
plt.title("Elo distribution after 30 games played in pairs with players added over time, when a player is added, anly that player plays against the existing players.")
plt.show()